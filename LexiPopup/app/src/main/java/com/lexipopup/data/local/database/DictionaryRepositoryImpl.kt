...
class DictionaryRepositoryImpl @Inject constructor(
    ...
    override suspend fun getSeedWords(query: String, limit: Int, mode: AppMode): List<WordEntry> =
        wordDao.getSeedWords(query, limit, mode.id).map { it.toDomain(gson) }

    override suspend fun getSeedWordCount(mode: AppMode): Int = wordDao.getSeedWordCount(mode.id)
}
...
fun WordEntity.toDomain(gson: Gson): WordEntry {
    return WordEntry(
        ...
        synonyms = runCatching { gson.fromJson<List<String>>(synonyms, STRING_LIST_TYPE) }.getOrNull() ?: emptyList(),
        antonyms = runCatching { gson.fromJson<List<String>>(antonyms, STRING_LIST_TYPE) }.getOrNull() ?: emptyList(),
        etymology = etymology,
